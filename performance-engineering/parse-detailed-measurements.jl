...
function plot_dfs(dfs, savename)
    ...
    markers = filter(m -> m ∈ Plots.supported_markers() &&
            m ∉ [:cross, :xcross], Plots._shape_keys) |>
              Iterators.cycle
...
function get_total_minus_sum(dfs)
    partial_dfs = [select(t.df, "Nthreads", "Total")
                   for t in dfs
                   if t.funcname != "Total Time"]
    total_times = [select(t.df, "Nthreads", "Total")
                   for t in dfs
                   if t.funcname == "Total Time"][1]
    ...
    time_sums = DataFrame(
        ...
        :TotalSum => sum([joined[:, n]
                          for n in names(joined, r"Total")]))

    two_totals = innerjoin(total_times, time_sums, on="Nthreads")

    DataFrame(:Nthreads => two_totals.Nthreads,
...
function main(directory)
    ...
    print("Difference between the total time measured\n and the sum of all the other measured times:\n")
