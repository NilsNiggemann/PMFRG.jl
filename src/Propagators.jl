...
"""
function iG_(gamma::AbstractArray, x::Integer, T::Real, nw::Integer, Λ::Real)
    w = get_w(nw)
...
"""
function iS_(gamma::AbstractArray, x::Integer, T::Real, nw::Integer, Λ::Real)
    w = get_w(nw)
    # return -θ(T) * w / 2 * iG_(gamma, x, T, nw)^2
    return -(T^2*w^2 - 3*Λ^2)/(2. *T^2.5*w) * iG_(gamma, x, T, nw, Λ)^2
end
...
function iSKat_(
    ...
    w = get_w(nw)
    return -((T^2*w^2 - 3*Λ^2)/(2. *T^2.5*w) + gamma_(Dgamma, x, nw)) * iG_(gamma, x, T, nw, Λ)^2
end
